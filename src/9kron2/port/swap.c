...
loop:
	...
	while(needpages(junk)) {
		/*
		 * The next if did run only when swap was configured,
		 * but swap does not work anyway and we want to steal
		 * text pages if that's feasible. So it's if(1) now.
		 */
		if(1) {
			x++;
			if((p = psincref(x)) == nil){
		...
		if(needpages(junk)){
			...
			print("no memory (%uld 4K; %uld 2M; %d procs)\n",
				palloc.freecount, bigpalloc.freecount, nprocs());
			ndnr();
